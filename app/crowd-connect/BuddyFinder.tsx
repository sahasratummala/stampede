...
import { 
  ...
  Loader2, Plus, Camera, Sparkles, Music
} from 'lucide-react';
  ...
  return (
    <div className="flex flex-col items-center justify-center animate-in fade-in duration-500 max-w-xl mx-auto">
      <div className="w-full space-y-8 text-center">
        {/* Profile Photo with Orange Fallback */}
...
export default function CrowdConnect() {
  const [view, setView] = useState<'connect' | 'matches' | 'profile'>('connect');
  ...
  const fetchBuddies = async (uid: string) => {
    setIsLoading(true);
    // Fetch IDs of people already liked to exclude them
    ...
    // Filter out: Yourself AND anyone you already liked
    ...
    if (error) console.error("Error fetching buddies:", error);
  ...
  const fetchMatches = async (uid: string) => {
    const { data, error } = await supabase
      .from('buddy_matches')
      .select(`target_id, buddies!buddy_matches_target_id_fkey (*)`)
      .eq('user_id', uid);
  ...
  const handleSaveProfile = async () => {
    ...
    if (!error) {
      ...
      setView('connect'); // Take them to Connect tab
      setCurrentIndex(0);
    } else {
  ...
  return (
    <div className="min-h-screen bg-black text-white pb-32 font-sans selection:bg-orange-500 selection:text-black">
      <div className="max-w-7xl mx-auto px-6 py-16">
        <h2 className="text-7xl font-black italic uppercase tracking-tighter mb-16 text-center lg:text-left">CROWD CONNECT</h2>
            ...
            <div className="bg-zinc-900/50 rounded-3xl border border-zinc-800 p-2 sticky top-24 backdrop-blur-xl">
              {[
                ...
                <button 
                  ...
                  className={`flex items-center gap-4 w-full px-8 py-5 rounded-2xl font-black uppercase italic transition-all ${view === item.id ? 'bg-orange-500 text-black translate-x-2' : 'text-zinc-500 hover:text-white hover:bg-zinc-800/50'}`}
                >
                  <item.icon size={22} strokeWidth={3} /> {item.label}
                  {item.id === 'matches' && matches.length > 0 && (
                    <span className="ml-auto bg-white/20 px-2 py-0.5 rounded-lg text-[10px]">{matches.length}</span>
                  )}
                </button>
              ))}
            </div>
            ...
            {isLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {matches.length > 0 ? matches.map(m => (
                  <div key={m.id} className="bg-zinc-900 p-6 rounded-[2rem] border border-zinc-800 flex items-center gap-6 animate-in zoom-in duration-300">
                    ...
                    <div>
                      <h4 className="font-black uppercase italic text-xl leading-tight">{m.name}</h4>
                      <p className="text-xs text-orange-500 font-bold tracking-widest uppercase">{m.major}</p>
                    </div>
                  </div>
                )) : (
                  <div className="col-span-2 text-center py-32 opacity-20 italic uppercase font-black tracking-[0.4em]">Your Herd is Empty</div>
                )}
              </div>
            ) : currentUser ? (
              <div className="max-w-lg mx-auto space-y-8 animate-in slide-in-from-bottom-8 duration-500">
                {/* SWIPE CARD */}
                <div className="relative rounded-[3rem] overflow-hidden aspect-[3.5/5] border-[12px] border-zinc-900 shadow-2xl bg-zinc-900 group">
                  {currentUser.photo ? (
                    <img src={currentUser.photo} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" alt="" />
                  ) : (
                    <div className="w-full h-full bg-orange-500 flex items-center justify-center">
                       <UserIcon size={120} className="text-black opacity-80" />
                    </div>
                  )}
                  <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent p-10 flex flex-col justify-end">
                    ...
                    <div className="flex items-center gap-3 text-orange-500 font-black uppercase text-sm tracking-widest">
                      <GraduationCap size={20}/> {currentUser.major}
                    </div>
                  </div>
                </div>

                {/* INFO SECTION */}
                <div className="bg-zinc-900 rounded-[2.5rem] p-10 border border-zinc-800 shadow-xl space-y-6">
                  <div>
                    <h4 className="text-[10px] text-zinc-500 font-black uppercase tracking-[0.3em] mb-3 flex items-center gap-2">
                      <Sparkles size={14} /> The Vibe
                    </h4>
                    <p className="text-2xl italic opacity-90 font-medium leading-relaxed">"{currentUser.bio || 'Silence is my vibe...'}"</p>
                  </div>
                  
                  {currentUser.interests?.length > 0 && (
                    <div className="flex flex-wrap gap-2 pt-4 border-t border-zinc-800">
                      {currentUser.interests.map((tag: string, i: number) => (
                        <span key={i} className="bg-zinc-800 text-white px-5 py-2.5 rounded-full text-[10px] font-black uppercase border border-white/5 tracking-wider">
                          {tag}
                        </span>
                      ))}
                    </div>
                ...
                <div className="flex gap-4">
                  <button onClick={() => handleLike(currentUser.id)} className="flex-[2] bg-orange-500 text-black font-black py-7 rounded-[2rem] text-3xl uppercase italic hover:scale-[1.03] active:scale-95 transition-all shadow-2xl shadow-orange-500/40">
                    Add to Herd
                  </button>
                  <button onClick={() => setCurrentIndex(prev => prev + 1)} className="flex-1 bg-zinc-900 text-zinc-500 font-black py-7 rounded-[2rem] text-2xl uppercase italic border-2 border-zinc-800 hover:text-white hover:border-zinc-700 transition-all">
                    Skip
                  </button>
                </div>
              </div>
            ) : (
              <div className="text-center py-40 bg-zinc-900/30 rounded-[3rem] border-4 border-dashed border-zinc-900 animate-pulse">
                <Sparkles size={80} className="mx-auto mb-6 text-orange-500 opacity-50" />
                <h3 className="text-4xl font-black uppercase italic tracking-widest opacity-30">Out of Buddies</h3>
              </div>
            )}
          </div>
        </div>
